...
Use the node to be checked as the upstream of the outbound chain, needs to be used in conjunction with the `detour_redir` of [Dial Fields](/configuration/shared/dial).

Let's say we have an outbound chain:
...
If we set `detour_of` of `C` to `A`, and ensure that the outbound `B` is configured to allow upstream redirection (as shown in the diagram above), then the actual outbound chain used by the health check is:
...
That is, what we test is the availability of a node as the upstream of the chain, so that the most reliable healthcheck data can be obtained and the hijacking of health check requests by some service providers can be avoided.

This option will be ignored if no outbound allowing `detour_redir` is found in the chain.
